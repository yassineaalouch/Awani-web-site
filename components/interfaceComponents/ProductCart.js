...
const ProductCard = ({ product, exchangeRate, currencyWanted }) => {
  ...
  return (
    <div className="border rounded-lg shadow-md p-1 md:p-4 hover:shadow-lg transition-shadow duration-300 flex flex-col md:max-w-64">
      ...
      <div className="flex flex-row md:flex-col w-full gap-4 md:gap-0">
        ...
        <div className="rounded-t-lg flex justify-center items-center md:w-full w-1/3">
          <div className="relative w-24 h-24 md:w-52 md:h-52">
            ...
            <Image
              ...
              className={
                animation
                  ? "object-contain z-50 absolute bg-cover inset-0 rounded-t-lg animated"
                  : "object-contain z-30 hidden absolute inset-0 rounded-t-lg"
              }
            />
          </div>
        </div>
        ...
        <div className="flex flex-col md:mt-4 justify-between w-2/3 md:w-full">
          ...
          <div className="flex flex-col">
            <h3 className="text-sm md:text-lg text-right font-semibold line-clamp-2 text-gray-800">
              {product?.title}
            </h3>
              ...
              {product?.discountPrice && (
                ...
                >
                  {formatCurrency({
                    number:
                      exchangeRate != null
                        ? product?.discountPrice * exchangeRate
                        : product?.discountPrice,
                    currencySymbol: currencyWanted,
                  })}
                </p>
          ...
          >
            {ProductExiste && existingProduct.quantity >= 1 ? (
              <div className="flex items-center justify-around w-full">
                ...
                >
                  <FaMinus
                    className="cursor-pointer h-full w-3 hover:text-red-500"
                    size={16}
                  />
                </div>
                <span className="text-[#6bb41e] font-extrabold text-base md:text-lg">
