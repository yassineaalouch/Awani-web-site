...
export default function AdminPage() {
  ...
  function fetchAdmins() {
    axios.get('/api/BlackList',{ headers: {
      ...
      .catch(err => console.error("Error fetching Black List: ", err));
  }

  function addAdmin(ev) {
    ev.preventDefault();
    axios.post('/api/BlackList', { email },{ headers: {
      'Authorization': `Bearer ${process.env.NEXT_PUBLIC_API_KEY_PROTECTION}`, // Envoyer l'API Key
    }})
      .then(result => {
  ...
  function editAdmin(ev) {
    ev.preventDefault();
    axios.put('/api/BlackList', { email: editingEmail, newEmail: email },{ headers: {
      'Authorization': `Bearer ${process.env.NEXT_PUBLIC_API_KEY_PROTECTION}`, // Envoyer l'API Key
    }})
      .then(result => {
  ...
  function deleteAdmin(email) {
    axios.delete('/api/BlackList', { data: { email }, headers: {
      'Authorization': `Bearer ${process.env.NEXT_PUBLIC_API_KEY_PROTECTION}`, // Envoyer l'API Key
    } })
      .then(() => {
  ...
  return (
    <Layout>
      <div className="flex flex-col">
          <h1 className="this">Black List page</h1>
          <form onSubmit={editingEmail ? editAdmin : addAdmin}>
            <label className="this">{editingEmail ? "Edit email" : "Add an email"}</label>
