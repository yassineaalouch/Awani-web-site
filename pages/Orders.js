...
export default function Orders() {
  ...
  async function ChangeStatus(cart, _id, e, userId) {
    axios.put('/api/purchaseRequest', { status: e.target.value, _id }, {
      headers: {
        'Authorization': `Bearer ${process.env.NEXT_PUBLIC_API_KEY_PROTECTION}`, // Envoyer l'API Key
      }
    }).then(() => {
                        ...
                        <table className="bg-white w-full rounded-lg border-gray-400 shadow-lg border-[2px]">
                          ...
                          <thead>
                            <tr className="bg-yellow-100 text-gray-600 uppercase text-sm leading-normal">
                              ...
                              <th className="py-2 px-4 text-nowrap text-left">Product properties</th>
                          ...
                          <tbody className="text-gray-600 text-sm font-light">
                            {/* {purchaseRequest.cart.map((cart) => (
                              <tr key={cart.id}>
                                ...
                                <td className="py-2 px-4 text-left">{cart.productProperties}</td>
                            ...
                            {purchaseRequest.cart.map((cart) => (
                              <tr key={cart.id}>
                                ...
                                <td className="py-2 px-4 text-left">
                                  {Object.entries(cart.productProperties).map(([key, value]) => (
                                    <div key={key}>
